...
private final class JisiluXLSBundleToken {}
...
public enum JisiluXLSLoader {
    public static func loadLatest() throws -> LoadedJisiluXLS {
        var lastError: Error?
        var foundFolder = false

        for folderURL in folderURLs() {
            do {
                let latestFile = try latestDatedXLS(in: folderURL)
                foundFolder = true
    ...
    private static func folderURLs() -> [URL] {
        ...
        if let sourceResourceURL = Bundle(for: JisiluXLSBundleToken.self).resourceURL {
            // iOS Playground 只能稳定读取 Resources 内随包复制的数据文件。
            folderURLs.append(
                sourceResourceURL.appendingPathComponent("Jisilu", isDirectory: true)
            )
            folderURLs.append(sourceResourceURL)
        }

        if let resourceURL = Bundle.main.resourceURL {
            // 与 100 Page 的 Resources/Stock 加载方式保持一致。
            folderURLs.append(resourceURL.appendingPathComponent("Jisilu", isDirectory: true))
